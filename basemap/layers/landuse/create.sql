...
CREATE
    MATERIALIZED VIEW IF NOT EXISTS osm_landuse_z12 AS WITH filtered AS(
        SELECT
            tags -> 'landuse' AS tag,
            st_buffer(
...
CREATE
    MATERIALIZED VIEW IF NOT EXISTS osm_landuse_z11 AS WITH filtered AS(
        SELECT
            tags -> 'landuse' AS tag,
            st_buffer(
                st_simplifypreservetopology(
...
CREATE
    MATERIALIZED VIEW IF NOT EXISTS osm_landuse_z10 AS WITH filtered AS(
        SELECT
            tags -> 'landuse' AS tag,
            st_buffer(
                st_simplifypreservetopology(
